	...
	class Easing {
		...
		static animate(item, time, initialProps, finalProps, duration, onFinish) {
			...
			for (let prop in initialProps) {
				...
				// For the filters which are arrays we will change every value and then reformulate the array
				if (Array.isArray(from)) {
					let fromCounter, fromProp, fromValue, toValue, newValue, fromItem, toItem, values = [];
					for (fromCounter=0; fromCounter<from.length; fromCounter++) {
						fromItem = from[fromCounter];
						toItem = to[fromCounter];
						for (fromProp in fromItem) {
							fromValue = fromItem[fromProp];
							toValue = toItem[fromProp];
							if (fromValue.toString().split('px').length > 1) {
								subfix = 'px';
							} else if (fromValue.toString().split('%').length > 1) {
								subfix = '%';
							}
							values.push(fromProp + '(' + Easing.easeOutQuart(time, fromValue, toValue, duration) + subfix + ')');
						}	
					}
					value = values.join(' ');
					console.log(value);
				} else {
					if (from.toString().split('px').length > 1) { subfix = 'px'; }
					value = Easing.easeOutQuart(time, from, to, duration) + subfix;
				}
	...
	class StackGallery {
		...
		nextItem() {
			if (!this.isAnimating) {
				...
				MagicUtils.updateStyle('newImage', newImage, MagicStyles.stackImage());
				...
				Easing.animate(oldTopElement, 0, {
						...
						'marginTop': -Math.ceil(this.margin/2) + 'px'
					},
					400,
					function () {
						setTimeout(function() {
				...
				// To correct a bug related to animation and the disappear effect on the top element of the stack
				oldTopElement.style.zIndex = (this.initialZIndex + this.visibleItems + 1);
			}
		}
		...
		draw() {
			...
			for (itemCounter = 0; itemCounter < this.itemsData.length; itemCounter++) {
				...
				if (this.topItem == itemCounter) {
					...
					MagicUtils.updateStyle('image', image, MagicStyles.stackImage());
			...
			MagicUtils.animateText(this.homeButton, "HOME", () => {
				...
				MagicUtils.animateText(this.aboutMeButton, "ABOUT ME", () => {
					...
					MagicUtils.animateText(this.thisExperimentButton, "THIS EXPERIMENT", () => {
						...
						let facebookButton = MagicUtils.createNode('a', 'home-button');
						facebookButton.href = 'https://www.facebook.com/muchologo';
						facebookButton.addEventListener("click", this.loadHome.bind(this));
						this.header.appendChild(facebookButton);
						MagicUtils.updateStyle('facebookButton', facebookButton, {...MagicStyles.social(), ...MagicStyles.facebook()});
						MagicUtils.animateText(facebookButton, "facebook");

						let twitterButton = MagicUtils.createNode('a', 'home-button');
						twitterButton.href = 'https://twitter.com/jaimezaballa';
						twitterButton.addEventListener("click", this.loadHome.bind(this));
						this.header.appendChild(twitterButton);
						MagicUtils.updateStyle('twitterButton', twitterButton, {...MagicStyles.social(), ...MagicStyles.twitter()});
						MagicUtils.animateText(twitterButton, "twitter", () => {
							this.loadHome();
		...
		loadThisExperiment() {
			if (this.mainContainer.childElementCount>0) {
				...
				Easing.animate(this.mainContainer, 0, {'opacity': 0}, {'opacity':1}, 400, () => {
					...
					let featuresList = [
						'More than 1000 lines of code! (well some empty lines XD)',
					...
					let improvementsList = [
						...
						"Create a section to explain the creation of every component :)",
